...
LoRa supports bi-directional communications with maximum payload size of 242
...
feature of the OS.
It allow to defines parts of the network stack we are using in a Contiki
...
Charlier M. in~\cite{uwbtsch} explores the requirements to adapt TSCH for the Ultra
Wideband (UWB). 
Similarly to LoRa, UWB do not allow CCA and can not use CSMA for its 
communications.
...
This allows neighbors to synchronize their clock and broadcast messages to the
...
their own information to the packet they are relaying until the gateway is
reached.
The clock drift is mitigated by synchronizing on the preamble but the receiver
do not acknowledge the reception of the packet, making the network sensible to
interference since it is running on a single channel and SF.

\paragraph{}

The following study look at ways to create fully functioning multi-hop network
...
There is no scheduling of which node transmits, it can happen that two nodes
transmit at the same time.

% Every nodes of the network keep track of their distance to the sink

Also, LoRaBlink use fixed SF, BW and channel. This decreases the overall throughput 
...
account the SF when computing best routes.
