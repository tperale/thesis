...
bandwidth increases the noise floor (\ref{eq:noisefloorbw})which reduce the
...
following parameters for each time slot cell.
...
router}.
RPL give each node a rank based on the distance from the root node. 
...
networks where each node autonomously compute their schedules without the
need of a scheduler~\cite{duquennoy2015}.
Each node maintains their schedule and keep track of their neighbor solely
...
and channel offset using the sender and receiver identification as parameters,
Orchestra achieve a very low level of collisions~\cite{duquennoy2015}.
