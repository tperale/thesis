...
from range issue.
Research has shown that in large LoRaWAN network collisions often occur
because of the congestion on the same spreading factors and the same channel.
Also, motes that are on the edge of the gateway coverage struggle to achieve a
complete transmission.

To solve these issues this thesis uses the existing multi-hop routing
protocols with LoRa (like RPL or Orchestra) to increase the reliability
and range of the network.
But as of today LoRa still misses power efficient MAC protocol that suits the
constraints of availability in multi-hop routing communications.
Time-Slotted Channel Hopping (TSCH) is already used in IEEE802.15.4e already
prooved its effiency with multi-hop networks but still lack of a port for LoRa.
This thesis will cover the adaptation of Time-Slotted Channel Hopping (TSCH)
MAC protocol for LoRa radio.
To do that, I will implement a radio driver for the RN2483 LoRa radio
module and adapt the current implementation of TSCH in Contiki to work with
it.
Based on my work I will show that LoRa and TSCH work seamlessly together
with the upper-layer protocols available in Contiki.
I will do this by testing my implementation against jammed radio channels and
by sending packets over multiple hops and thus prooving adapting TSCH enable
power efficient et reliable multi-hop networks in LoRa.
...
The goal of the thesis aimed to adapt TSCH to work with LoRa and thus achieving
power-efficient and reliable multihop networking with LoRa radio on Contiki OS with
the RN2483 module.
...
module that served as a basis for the next part.
In the second part, I detailed the TSCH timing parameters as well as the driver
parameters needed by TSCH to successfully adapt TSCH to work with LoRa by
...
% Real world testing on the range of the network were not conducted due to the
% lockdown measure in place during the time I wrote my thesis.

To achieve longer distance transmission, the ideal implementation would adapt
its SF depending on the distance between two hops.
Even though the different SF are supported in my implementation, only
a single SF is used at the same time.
A lot of LoRa features are lost by using a single SF.
Supporting multiple SF at the same time would increase the network throughput
and increase the transmission range for nodes with low reception.
...
\chapter*{Further work\label{section:further}}
