...
module. 
The section~\ref{section:pingpong} detail how I ran a ping pong
example based on that driver to proove the driver is fully working with the
...
I finaly managed to bring TSCH to LoRa in~\ref{section:tschtesting} by
demonstating TSCH and LoRa integrate to the 6TiSCH networking stack.
...
Analysis of the accuracy and reliability of the network will then be possible.
A study on how longer sleeping period de-synchronize the network or the period
needed between EB packets, could be conducted.

\paragraph{}

The ideal implementation would adapt its SF depending on the distance
between two hops.
In my implementation even though the different SF are supported, it operate
on a single SF at the time. 
We miss a lot of LoRa features with the single SF, supporting multiple SF at
the same time would increase the network troughput and increase the
transmission range for nodes with low reception.
The litterature don't really talk about using adaptive SF in a multihops
network except for~\cite{8115756} that could give us indications on the future
work on the subject.

\paragraph{}

Related work reinveted the wheel by implementing their own multi-hop protocol
on top of LoRa.
Using TSCH, an already well studied MAC protocol, and its ability to
communicate over multiple channel with time-slots for better power usage 
represent a step forward in the journey to achieve multi-hop communication with
LoRa.
Using every SF to use the full LoRa capacity is the future challenge for this
kind of work.
